<template>
  ...
  <div class="base-container">
    <div class="container">
      ...
      <div>
        <QuillEditor v-model="blogContent"></QuillEditor>
      </div>
      <div class="blog-actions mt-3">
        <button  @click="updateBlog" class="me-3">Gem ændringer</button>
        <RouterLink to="postPreview">Se ændringer</RouterLink>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  async mounted() {
    ...
    this.currentBlog = await this.$store.state.allBlogPosts.filter((post) => {
    ...
    openPreview() {
     ...
     async updateBlog() {
      //TODO  cursor location
       ...
       const docRef = doc(collectionRef, this.routeID);
